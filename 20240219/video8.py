...
class Ui_MainWindow(QtWidgets.QWidget):
    ...
    def play_video(self):
        ...
        if text2:  # 입력된 텍스트가 있는지 확인
            ...
            conn.commit() # 저장
            conn.close() 
            self.th2 = Thread2(self.lineEdit_2, recording_file2)  # 스레드 생성 및 시작
...
class Thread(QThread):
    ...
    def run(self):
        ...
        while self.isRunning:
            ret, frame = cap.read()
            if ret:
                results = model.predict(frame, save=False, imgsz=640, conf=0.2,
                ...
                scaled_pixmap = pixmap.scaled(527, 230, Qt.IgnoreAspectRatio)  # 이미지 크기를 527x230 픽셀로 조절
